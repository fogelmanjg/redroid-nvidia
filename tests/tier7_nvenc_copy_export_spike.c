/* Tier 7 spike 3: does a dma_buf fd exported by ONE VkDevice with ONLY
 * DMA_BUF_BIT_EXT (simulating Venus's own resource-creation code, which this
 * project hasn't touched and would rather not touch) still reach NVENC if a
 * completely SEPARATE, independent VkDevice/VkInstance ("the encode side")
 * consumes it - not by re-exporting the SAME imported memory (spike 2,
 * confirmed broken - see tier7_nvenc_dualexport_spike.c's sibling result and
 * README.md), but by plain-importing it (self-compatible, confirmed correct)
 * and then doing a real GPU-to-GPU vkCmdCopyImage into a second, freshly
 * *self*-allocated image the encode side owns outright (same dual-export
 * shape spike 1 already proved works)?
 *
 * If yes: a new vtest command wrapping virgl_renderer_resource_export_blob()
 * (which already exists in virglrenderer's public API, no protocol/resource-
 * creation changes needed) can encode ANY resource Venus is already
 * rendering into - including SurfaceFlinger's real render target - without
 * touching Venus's own image-creation code at all, at the cost of one real
 * GPU copy per frame (both images stay OPTIMAL/tiled throughout - this never
 * touches Tier 5's broken LINEAR/CPU-mappable path).
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(void)
{
    ...
    /* Step A: plain, spec-compliant import - ONLY DMA_BUF_BIT_EXT, no export
     * chained. This is self-compatible per the diagnostic above and should
     * correctly alias the producer's real memory. */
    const VkExternalMemoryImageCreateInfo imported_ext = { .sType = VK_STRUCTURE_TYPE_EXTERNAL_MEMORY_IMAGE_CREATE_INFO, .handleTypes = VK_EXTERNAL_MEMORY_HANDLE_TYPE_DMA_BUF_BIT_EXT };
    const VkImageDrmFormatModifierExplicitCreateInfoEXT explicit_mod = { .sType = VK_STRUCTURE_TYPE_IMAGE_DRM_FORMAT_MODIFIER_EXPLICIT_CREATE_INFO_EXT, .pNext = &imported_ext, .drmFormatModifier = chosen.drmFormatModifier, .drmFormatModifierPlaneCount = 1, .pPlaneLayouts = &layout };
    ...
    CHK_VK(vkCreateImage(cons.dev, &imported_imginfo, NULL, &imported_image));

    VkMemoryRequirements imported_reqs;
    vkGetImageMemoryRequirements(cons.dev, imported_image, &imported_reqs);
    uint32_t imported_mem_type = UINT32_MAX;
    for (uint32_t i = 0; i < cons.mem_props.memoryTypeCount; i++)
        if ((imported_reqs.memoryTypeBits & (1u << i)) && (cons.mem_props.memoryTypes[i].propertyFlags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT))
            { imported_mem_type = i; break; }
    if (imported_mem_type == UINT32_MAX) imported_mem_type = __builtin_ffs(imported_reqs.memoryTypeBits) - 1;
    ...
    const VkMemoryDedicatedAllocateInfo imported_ded = { .sType = VK_STRUCTURE_TYPE_MEMORY_DEDICATED_ALLOCATE_INFO, .pNext = &import_info, .image = imported_image };
    const VkMemoryAllocateInfo imported_alloc = { .sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO, .pNext = &imported_ded, .allocationSize = imported_reqs.size, .memoryTypeIndex = imported_mem_type };
    VkDeviceMemory imported_memory;
    CHK_VK(vkAllocateMemory(cons.dev, &imported_alloc, NULL, &imported_memory));
    CHK_VK(vkBindImageMemory(cons.dev, imported_image, imported_memory, 0));
    fprintf(stderr, "[consumer] plain-imported the producer's fd (DMA_BUF_BIT_EXT only, no re-export declared)\n");

    /* Step B: a fresh, consumer-OWNED image with combined handle types -
     * exactly spike 1's proven-working single-owner pattern. */
    const VkExternalMemoryImageCreateInfo copy_ext = { .sType = VK_STRUCTURE_TYPE_EXTERNAL_MEMORY_IMAGE_CREATE_INFO, .handleTypes = VK_EXTERNAL_MEMORY_HANDLE_TYPE_DMA_BUF_BIT_EXT | VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT_KHR };
    const VkImageDrmFormatModifierListCreateInfoEXT copy_modlist = { .sType = VK_STRUCTURE_TYPE_IMAGE_DRM_FORMAT_MODIFIER_LIST_CREATE_INFO_EXT, .pNext = &copy_ext, .drmFormatModifierCount = mod_count, .pDrmFormatModifiers = mod_candidates };
    const VkImageCreateInfo copy_imginfo = {
        .sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO, .pNext = &copy_modlist,
        .imageType = VK_IMAGE_TYPE_2D, .format = format, .extent = { W, H, 1 },
        .mipLevels = 1, .arrayLayers = 1, .samples = VK_SAMPLE_COUNT_1_BIT,
        .tiling = VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT,
    ...
    VkImage copy_image;
    CHK_VK(vkCreateImage(cons.dev, &copy_imginfo, NULL, &copy_image));

    VkMemoryRequirements cons_reqs;
    vkGetImageMemoryRequirements(cons.dev, copy_image, &cons_reqs);
    ...
    const VkExportMemoryAllocateInfo cons_exp = { .sType = VK_STRUCTURE_TYPE_EXPORT_MEMORY_ALLOCATE_INFO, .handleTypes = VK_EXTERNAL_MEMORY_HANDLE_TYPE_DMA_BUF_BIT_EXT | VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT_KHR };
    const VkMemoryDedicatedAllocateInfo cons_ded = { .sType = VK_STRUCTURE_TYPE_MEMORY_DEDICATED_ALLOCATE_INFO, .pNext = &cons_exp, .image = copy_image };
    ...
    CHK_VK(vkAllocateMemory(cons.dev, &cons_alloc, NULL, &cons_memory));
    CHK_VK(vkBindImageMemory(cons.dev, copy_image, cons_memory, 0));
    fprintf(stderr, "[consumer] allocated its OWN fresh dual-export image for the copy target\n");

    /* Step C: a real GPU-to-GPU copy, both images OPTIMAL/tiled throughout -
     * never touches Tier 5's broken LINEAR/CPU-mappable path at all. */
    VkCommandPool cons_pool;
    const VkCommandPoolCreateInfo cons_poolinfo = { .sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO, .queueFamilyIndex = 0 };
    CHK_VK(vkCreateCommandPool(cons.dev, &cons_poolinfo, NULL, &cons_pool));
    VkCommandBuffer cons_cmd;
    const VkCommandBufferAllocateInfo cons_cbinfo = { .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO, .commandPool = cons_pool, .level = VK_COMMAND_BUFFER_LEVEL_PRIMARY, .commandBufferCount = 1 };
    CHK_VK(vkAllocateCommandBuffers(cons.dev, &cons_cbinfo, &cons_cmd));
    const VkCommandBufferBeginInfo cons_begininfo = { .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO };
    CHK_VK(vkBeginCommandBuffer(cons_cmd, &cons_begininfo));

    /* Imported DRM-modifier-tiled external image: UNDEFINED->TRANSFER_SRC_OPTIMAL
     * preserves the real tiled bytes underneath (standard for foreign/imported
     * dma-buf images - the synthetic Vulkan "layout" is just barrier bookkeeping,
     * not a content-defining property, for explicit-modifier external images). */
    VkImageMemoryBarrier imported_to_src = {
        .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, .oldLayout = VK_IMAGE_LAYOUT_UNDEFINED, .newLayout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
        .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .image = imported_image,
        .subresourceRange = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1 }, .dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT,
    };
    VkImageMemoryBarrier copy_to_dst = {
        .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, .oldLayout = VK_IMAGE_LAYOUT_UNDEFINED, .newLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .image = copy_image,
        .subresourceRange = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1 }, .dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT,
    };
    VkImageMemoryBarrier both_barriers[2] = { imported_to_src, copy_to_dst };
    vkCmdPipelineBarrier(cons_cmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 0, NULL, 2, both_barriers);
    ...
    vkCmdCopyImage(cons_cmd, imported_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, copy_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);

    VkImageMemoryBarrier copy_to_general = copy_to_dst;
    copy_to_general.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL; copy_to_general.newLayout = VK_IMAGE_LAYOUT_GENERAL;
    copy_to_general.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT; copy_to_general.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
    vkCmdPipelineBarrier(cons_cmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, 0, 0, NULL, 0, NULL, 1, &copy_to_general);
    CHK_VK(vkEndCommandBuffer(cons_cmd));
    const VkSubmitInfo cons_submit = { .sType = VK_STRUCTURE_TYPE_SUBMIT_INFO, .commandBufferCount = 1, .pCommandBuffers = &cons_cmd };
    CHK_VK(vkQueueSubmit(cons.queue, 1, &cons_submit, VK_NULL_HANDLE));
    CHK_VK(vkQueueWaitIdle(cons.queue));
    fprintf(stderr, "[consumer] real vkCmdCopyImage from the imported buffer into its own copy, both OPTIMAL throughout\n");

    /* Step D: export the copy's memory - self-allocated, self-compatible,
     * exactly spike 1's confirmed-working shape. */
    ...
    fprintf(stderr, "[consumer] exported the copy as opaque_fd=%d\n", opaque_fd);
...
#define MKVER(structver) (drv_api_version | ((uint32_t)(structver) << 16) | (0x7u << 28))
    ...
    FILE *f = fopen("/tmp/tier7_spike3_out.h264", "wb");
    ...
    fprintf(stderr, "plain-imported by a totally SEPARATE, independent Vulkan device, GPU-copied (both images\n");
    fprintf(stderr, "OPTIMAL throughout) into that device's own dual-export image, then encoded via NVENC.\n");
    fprintf(stderr, "This works for ANY Venus-owned resource, no Venus changes needed.\n");
    fprintf(stderr, "Wrote /tmp/tier7_spike3_out.h264 - decode and check the color is (30,200,90).\n");
