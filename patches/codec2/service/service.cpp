...
/*
 * redroid-nvidia Tier 7: registers c2.hardware.encoder.h264 for real,
 * createComponent()/createInterface() constructing an NvencEncComponent
 * whose process() calls VCMD_ENCODE_RESOURCE (host-side NVENC, confirmed
 * end to end - see redroid-nvidia's README.md/DEVLOG.md) instead of
 * returning C2_NOT_FOUND.
 *
 * Adapted from redroid-hwenc's own android.hardware.media.c2-vaapi-service
 * (external/vaapi_codec2/service/service.cpp in this same tree), itself
 * adapted from AOSP's official empty-service template at
 * frameworks/av/media/codec2/hal/services/vendor.cpp.
 *
 * Only one Codec2 IComponentStore may register as "default" at a time -
 * this service, redroid-hwenc's vaapi one, and AOSP's own stock default
 * service are mutually exclusive product choices, not meant to coexist in
 * the same device build (they happen to share this AOSP checkout because
 * both hardware-encoder projects were developed against it).
 */

//#define LOG_NDEBUG 0
#define LOG_TAG "android.hardware.media.c2-nvenc-service"
...
// "android.hardware.media.c2-nvenc-seccomp_policy" in Android.bp.
static constexpr char kBaseSeccompPolicyPath[] =
        "/vendor/etc/seccomp_policy/"
        "android.hardware.media.c2-nvenc-seccomp_policy";

// Additional seccomp permissions can be added in this file.
// This file does not exist by default.
static constexpr char kExtSeccompPolicyPath[] =
        "/vendor/etc/seccomp_policy/"
        "android.hardware.media.c2-nvenc-extended-seccomp_policy";
...
public:
    ...
    virtual C2String getName() const override {
        // Instance name matters: only specific patterns ("default[0-9]*",
        // "vendor[0-9]*_software") are accepted by the Framework
        // Compatibility Matrix / Codec2Client::CacheServiceNames() - see
        // redroid-hwenc's own vaapi service.cpp for the on-device
        // confirmation that a custom name is invisible to real apps even
        // though vintf accepts it fine.
        return "default";
    }
    ...
    virtual c2_status_t createComponent(
        ...
        auto intf = std::make_shared<android::NvencEncInterface>(mReflectorHelper);
        *component = std::make_shared<android::NvencEncComponent>(name.c_str(), mNextId++, intf);
        return C2_OK;
    }
    ...
    virtual c2_status_t createInterface(
        ...
        *interface = std::make_shared<
                android::SimpleInterface<android::NvencEncInterface>>(
...
int main(int /* argc */, char** /* argv */) {
    LOG(DEBUG) << "android.hardware.media.c2-nvenc-service starting...";
